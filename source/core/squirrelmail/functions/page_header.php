 ...
 *
 * $Id: page_header.php,v 1.1 2003-06-21 13:17:20 alex Exp $
 */
...
<!--
  ...
  body { 
      ...
      width: expression(document.documentComponent.clientWidth - 30);
  }
-->
</style>
...
function displayPageHeader($color, $mailbox, $xtra='', $session=false) {
    ...
    $application = substr( $PHP_SELF, ( strlen( $PHP_SELF ) - strlen( $base_uri ) ) * -1 );
    if ($qmark = strpos($application, '?')) {
        $application = substr($application, 0, $qmark);
    }
    if (!isset($frame_top)) {
        $frame_top = '_top';
    }
