 ...
 *
 * $Id: Message.class.php,v 1.1 2003-06-21 13:17:16 alex Exp $
 */
        ...
        for ($cnt = strlen($read); $i < $cnt; ++$i) {
            ...
            switch ($char) {
                case '(':
                    switch($arg_no) {
                        ...
                        case 1:
                            ...
                            ++$i;
                            $arg_a[] = $message->parseProperties($read, $i);
                            ++$arg_no;
                            break;
                        case 2:
                            if (isset($msg->type0) && ($msg->type0 == 'multipart')) {
                                ++$i;
                                $arg_a[] = $message->parseDisposition($read, $i);
                            } else { /* properties */
                                $arg_a[] = $message->parseProperties($read, $i);
                            }
                            ++$arg_no;
                        ...
                        case 3:
                            if (isset($msg->type0) && ($msg->type0 == 'multipart')) {
                                ++$i;
                                $arg_a[]= $message->parseLanguage($read, $i);
                            }
                        case 7:
                            if (($arg_a[0] == 'message') && ($arg_a[1] == 'rfc822')) {
                                ...
                                $msg->rfc822_header = $message->parseEnvelope($read, $i, $rfc822_hdr);
                                while (($i < $cnt) && ($read{$i} != '(')) {
                                    ++$i;
                                }
                                $msg->addEntity($message->parseBodyStructure($read, $i,$msg));
                       ...
                       case 10:
                           if (($arg_a[0] == 'text') || (($arg_a[0] == 'message') && ($arg_a[1] == 'rfc822'))) {
                               ...
                               $i = $message->parseParenthesis($read, $i);
                ...
                case '"':
                    ...
                    $arg_s = $message->parseQuote($read, $i);
                    ++$arg_no;
                    if ($arg_no < 3) {
                ...
                case '{':
                    ...
                    $arg_s = $message->parseLiteral($read, $i);
                    ++$arg_no;
                    break;
		case '0':    
